...
for node in range(1,21):
    if node!=54: 
        print(node)
        RNN=pd.read_csv('RNNweight'+str(node)+'/Weightweather Lmst.csv.csv')
        GRU=pd.read_csv('GRUweight'+str(node)+'/weight.csv')
...
RNN=pd.read_csv('level3/RNN of weight 2'+str(1)+'/weight_level3_LSTM.csv')
RGU=pd.read_csv('level3/GRU of weight 2'+str(1)+'/weight_level1_GRU_.csv')
CNN=pd.read_csv('level3/CNN of weight 2'+str(1)+'/weight_CNN_.csv')
rnn=RNN.loss
rgu=RGU.loss
cnn=CNN.loss
